    ...
    const html = `
      <div style="font-family:Arial, Helvetica, sans-serif; color:#111;">
        <div style="max-width:680px;margin:0 auto;padding:24px;border-radius:8px;background:#fff;">
          <h2 style="margin:0 0 8px 0;color:#0b3d91;">Password Reset Verification</h2>
          <p style="margin:0 0 16px 0;color:#333;">
            Hello ${(userDoc as any).name ?? ""},<br/>
            You requested to change your password. Use the code below to continue. This code will expire in <strong>15 minutes</strong>.
          </p>
          ...
          <div style="margin:20px 0;padding:16px;border-radius:8px;background:#f5f7ff;display:flex;align-items:center;justify-content:center;">
            <span style="font-size:28px;letter-spacing:4px;font-weight:700;color:#0b3d91;">${resetCode}</span>
          </div>

          <p style="margin:0 0 8px 0;color:#666;">
            Or open the app and enter the code manually. If you did not request a password reset, please ignore this email.
          </p>
          ...
          <p style="font-size:12px;color:#999;margin:0;">
            Sent from <a href="${appUrl}" style="color:#0b3d91;text-decoration:none">${appUrl}</a>
          </p>
        </div>
      </div>
    `;
