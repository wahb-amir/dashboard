...
export default function ProjectDashboard({ setIsQuoteModalOpen, projectCount }) {
    ...
    // Editable statuses (buttons active only when status is one of these)
    ...
    const handleSetNewProject = useCallback((newProjectData) => {
        ...
        const transformed = {
            ...
            name,
            status,
            currentLead: `Lead TBD (Request by ${newProjectData.name || newProjectData.contactName || 'unknown'})`,
            currentTask: `Awaiting initial review. ${newProjectData.details ? newProjectData.details.slice(0, 100) + '...' : 'No details provided.'}`,
            lastUpdate: 'Just now',
            timeline: [
                { step: "Project Created", date: new Date().toLocaleString(), status: "Quote Requested", notes: `Request submitted by ${newProjectData.name || 'unknown'}.` },
            ],
            data: newProjectData
        };
    ...
    // removed saveStepToBackend / handleAddStep - dev team will handle step updates
    ...
    const handleCancelProject = async (projectId) => {
        ...
        if (!editableStatuses.includes(projectToCancel.status)) {
            toast.error('Project cannot be cancelled. Please contact dev.buttnetworks@gmail.com');
            return;
        }
                            ...
                            <motion.div key={currentProject.id} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -30 }}>
                                <div className="flex items-start justify-between">
                                    ...
                                    <div className="flex items-center gap-3">
                                        {editableStatuses.includes(currentProject?.status) ? (
                                            <>
                                                <ConfirmModal
                                            ...
                                            <div className="text-sm text-gray-600">
                                                Modifications locked — contact <a className="text-blue-600 underline" href="mailto:dev.buttnetworks@gmail.com">dev.buttnetworks@gmail.com</a>
                                            </div>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                                        ...
                                        {Array.isArray(currentProject?.developers) && currentProject.developers.length > 0 ? (
                                            <div className="p-4 bg-white rounded-xl shadow border-t-4 border-green-500">
                                                ...
                                                {currentProject.developers.map((dev, index) => {
                                                    ...
                                                    return (
                                                        <a key={key} href={href} target="_blank" rel="noopener noreferrer" className="flex items-center mb-2 last:mb-0 hover:opacity-90">
                                                            <User className="w-5 h-5 mr-2 text-green-500" />
                                                            <span className="font-semibold text-gray-900">{displayName}<sup className='text-blue-800 text-sm ml-2'>dev</sup></span>
                                                        </a>
                                                    );
                                                })}
                                            </div>
                                    ...
                                    <div className="p-4 bg-white rounded-xl shadow border-t-4 border-gray-300">
                                        <p className="text-sm text-gray-500">Last Activity</p>
                                        <p className="text-lg font-bold text-gray-900 mt-1 flex items-center"><Clock className="w-5 h-5 mr-2 text-gray-500" /> {currentProject.lastUpdate}</p>
                                    </div>
                                </div>
